<template>
  <div class="px-4 pb-4 pt-2">
    <h2 class="mb-4 text-lg font-bold text-gray-900">Generate route</h2>

    <div v-if="selectedLat" class="mb-4 flex items-center gap-2 rounded-xl bg-green-50 px-3 py-2">
      <svg class="h-4 w-4 flex-shrink-0 text-green-600" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/>
      </svg>
      <span class="text-sm text-green-700">Start point set · tap map to change</span>
    </div>
    <p v-else class="mb-4 rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-600">
      Tap on the map to set your starting point
    </p>

    <div class="space-y-4">
      <div>
        <div class="mb-1 flex justify-between text-sm">
          <label class="font-medium text-gray-700">Distance</label>
          <span class="text-gray-500">{{ (form.distance_m / 1000).toFixed(1) }} km</span>
        </div>
        <input
          v-model.number="form.distance_m"
          type="range" min="500" max="15000" step="500"
          class="w-full accent-blue-600"
        />
        <div class="mt-1 flex justify-between text-xs text-gray-400">
          <span>0.5 km</span><span>15 km</span>
        </div>
      </div>

      <div>
        <div class="mb-1 flex justify-between text-sm">
          <label class="font-medium text-gray-700">Points of interest</label>
          <span class="text-gray-500">{{ form.num_pois }}</span>
        </div>
        <input
          v-model.number="form.num_pois"
          type="range" min="2" max="10" step="1"
          class="w-full accent-blue-600"
        />
        <div class="mt-1 flex justify-between text-xs text-gray-400">
          <span>2</span><span>10</span>
        </div>
      </div>

      <div class="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
        <span class="text-sm font-medium text-gray-700">Circular route</span>
        <button
          type="button"
          class="relative inline-flex h-6 w-11 rounded-full transition-colors"
          :class="form.is_circular ? 'bg-blue-600' : 'bg-gray-300'"
          @click="form.is_circular = !form.is_circular"
        >
          <span
            class="inline-block h-5 w-5 translate-y-0.5 rounded-full bg-white shadow transition-transform"
            :class="form.is_circular ? 'translate-x-5' : 'translate-x-0.5'"
          />
        </button>
      </div>

      <button
        :disabled="!selectedLat || loading"
        class="w-full rounded-xl bg-blue-600 py-3.5 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40 min-h-[48px]"
        @click="submit"
      >
        <span v-if="loading" class="flex items-center justify-center gap-2">
          <svg class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
          </svg>
          Generating…
        </span>
        <span v-else>Generate route</span>
      </button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { reactive } from 'vue'

const props = defineProps<{
  selectedLat?: number | null
  selectedLon?: number | null
  loading: boolean
}>()

const emit = defineEmits<{
  generate: [req: { distance_m: number; num_pois: number; is_circular: boolean }]
}>()

const form = reactive({ distance_m: 3000, num_pois: 4, is_circular: true })

function submit() {
  if (!props.selectedLat) return
  emit('generate', { ...form })
}
</script>
